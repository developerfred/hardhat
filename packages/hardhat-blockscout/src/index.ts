...
import {
  ...
  verifyContract,
} from "./blockscout/BlockscoutService";
import {
  toCheckStatusRequest,
  toVerifyRequest,
} from "./blockscout/BlockscoutVerifyContractRequest";
import {
  BlockscoutURLs,
...
interface VerifyMinimumBuildArgs {
  ...
  etherscanAPIEndpoints:  BlockscoutURLs;
  address: string;
...
If your constructor has no arguments pass an empty array. E.g:
  ...
  const etherscanAPIEndpoints:  BlockscoutURLs = await run(
    TASK_VERIFY_GET_ETHERSCAN_ENDPOINT
  );
